...
class CalIndexer {
  ...
  public function __construct() {
    ...
    $this->view->setPartialRootPaths( [ 
        ...
        ExtensionManagementUtility::extPath( 'cal' ) . 'Resources/Private/Templates/Backend/IndexerModule/Partials/'
    ] );
    ...
    switch (intval( $this->request->getQueryParams() ['SET'] ['function'] )) {
      case 2 :
        ...
        if (count( $pageIds ) > 0 && is_int( $starttime ) && is_int( $endtime )) {
          ...
          $rgc = GeneralUtility::makeInstance( \TYPO3\CMS\Cal\Utility\RecurrenceGenerator::class, 0, $starttime, $endtime );
          ...
          if (! empty( $pages )) {
            ...
            $table [] = '<div class="form-group col-sm-6" id="pageIds_col">' . $label1 . '</div>';
            $table [] = '<div class="form-group col-sm-6" id="pageIds_colId">' . $label2 . '</div>';
            
            foreach ( $pages as $pageId => $pageTitle ) {
              
              $table [] = '<div class="form-group col-sm-6" id="pageIds_col' . $pageId . '">' . '<div class="form-control-wrap">' . '<div class="input-group" id="tceforms-pageIds_col' . $pageId . '_row-wrapper">' . $pageTitle . ' [' . $pageId . '] ' . '<input name="pageIds[]" value="' . $pageId . '" data-date-offset="0" type="hidden" id="tceforms-pageIds_colId' . $pageId . '_row">' . '</div>' . '</div>' . '</div>';
              $table [] = '<div class="form-group col-sm-6" id="tsPage_colId' . $pageId . '">' . '<div class="form-control-wrap">' . '<div class="input-group" id="tsPage_colId' . $pageId . '_row-wrapper">' . '<input name="tsPage[]" value="" class="form-control  t3js-clearable" data-date-type="date" data-date-offset="0" type="text" id="tceforms-tsPage_colId' . $pageId . '_row">' . '</div>' . '</div>' . '</div>';
            }
            ...
            $table [] = '<div class="form-section"><div class="row"><div class="form-group col-sm-6" id="index_start_col">' . $label . '<div class="form-control-wrap">' . '<div class="input-group" id="tceforms-datetimefield-starttime_row-wrapper">' . '<input name="starttime" value="' . $value . '" class="form-control t3js-clearable" data-date-type="date" data-date-offset="0" type="text" id="tceforms-datetimefield-starttime_row">' . '</div>' . '</div>' . '</div>';
            ...
            $table [] = '<div class="form-group col-sm-6" id="index_end_col">' . $label . '<div class="form-control-wrap">' . '<div class="input-group" id="tceforms-datetimefield-endtime_row-wrapper">' . '<input name="endtime" value="' . $value . '" class="form-control  t3js-clearable" data-date-type="date" data-date-offset="0" type="text" id="tceforms-datetimefield-endtime_row">' . '</div>' . '</div>' . '</div></div></div>';
  ...
  public static function getMessage($message, $type) {
    ...
    $flashMessage = GeneralUtility::makeInstance( 'TYPO3\\CMS\\Core\\Messaging\\FlashMessage', htmlspecialchars( $message ), '', $type, TRUE );
