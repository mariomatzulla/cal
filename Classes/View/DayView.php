...
class DayView extends \TYPO3\CMS\Cal\View\BaseView {
  ...
  public function drawDay(&$master_array, $getdate) {
    ...
    $dayTemplate = $this->templateService->getSubpart( $page, '###DAY_TEMPLATE###' );
    if ($dayTemplate == '') {
      $rems = Array ();
      return $this->finish( $page, $rems );
    }
    ...
    $loop_dof = $this->templateService->getSubpart( $dayTemplate, '###DAYSOFWEEK###' );
    ...
    if (is_array( $view_array [$getdate] ['-1'] )) {
      $loop_ad = $this->templateService->getSubpart( $dayTemplate, '###LOOPALLDAY###' );
