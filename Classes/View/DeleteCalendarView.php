...
class DeleteCalendarView extends \TYPO3\CMS\Cal\View\FeEditingBaseView {
  ...
  public function getFreeAndBusyUserMarker(& $template, & $sims, & $rems) {
    ...
    if (! empty( $user )) {
      $result = $GLOBALS ['TYPO3_DB']->exec_SELECTquery( 'fe_users.name', 'fe_users', 'uid IN (' . implode( ',', $user ) . ')' . $this->pageRepository->enableFields( 'fe_users' ) );
    ...
    if (! empty( $groups )) {
      $result = $GLOBALS ['TYPO3_DB']->exec_SELECTquery( 'fe_groups.title', 'fe_groups', 'uid IN (' . implode( ',', $groups ) . ')' . $this->pageRepository->enableFields( 'fe_groups' ) );
      while ( $row = $GLOBALS ['TYPO3_DB']->sql_fetch_assoc( $result ) ) {
  ...
  public function getOwnerMarker(& $template, & $sims, & $rems) {
    ...
    if (! empty( $group )) {
      $result = $GLOBALS ['TYPO3_DB']->exec_SELECTquery( 'fe_groups.title', 'fe_groups', 'uid IN (' . implode( ',', $group ) . ')' . $this->pageRepository->enableFields( 'fe_groups' ) );
      while ( $row = $GLOBALS ['TYPO3_DB']->sql_fetch_assoc( $result ) ) {
        $displaylist [] = $row ['title'];
      }
      $GLOBALS ['TYPO3_DB']->sql_free_result( $result );
