...
class FeEditingBaseView extends \TYPO3\CMS\Cal\View\BaseView {
  ...
  public function getFormStartMarker(& $template, & $sims, & $rems, & $wrapped) {
    ...
    $rems ['###FORM_START###'] = $this->templateService->getSubpart( $template, '###FORM_START###' );
  }

  public function getFormEndMarker(& $template, & $sims, & $rems, & $wrapped) {

    $temp = $this->templateService->getSubpart( $template, '###FORM_END###' );
    $temp_sims = array ();
  ...
  protected function getTranslationOptionsMarker(& $template, & $sims, & $rems) {
    ...
    if (false && $this->isEditMode && $this->rightsObj->isViewEnabled( 'translation' ) && $this->rightsObj->isAllowedTo( 'create', 'translation' )) {
      ...
      $GLOBALS ['TYPO3_DB']->sql_free_result( $result );
      $result = $GLOBALS ['TYPO3_DB']->exec_SELECTquery( 'sys_language_uid', $this->table, 'l18n_parent = ' . $this->object->getUid() . $this->pageRepository->enableFields( $this->table ), '', 'sys_language_uid ASC' );
      $inUseLangIds = array ();
      while ( $row = $GLOBALS ['TYPO3_DB']->sql_fetch_assoc( $result ) ) {
  ...
  public function getSharedMarker(& $template, & $sims, & $rems) {
    ...
    if ($this->isAllowed( 'shared' )) {
      ...
      $result = $GLOBALS ['TYPO3_DB']->exec_SELECTquery( '*', 'fe_groups', 'pid in (' . $this->conf ['pidList'] . ')' . $this->pageRepository->enableFields( 'fe_groups' ) );
      while ( $row = $GLOBALS ['TYPO3_DB']->sql_fetch_assoc( $result ) ) {
