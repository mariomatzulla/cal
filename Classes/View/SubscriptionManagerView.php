...
class SubscriptionManagerView extends \TYPO3\CMS\Cal\View\BaseView {
  ...
  function drawSubscriptionManager() {
    ...
    if ($eventUID > 0 && $email && $subscriptionHash) {
      ...
      if (is_object( $event )) {
        ...
        $status = $this->templateService->getSubpart( $page, '###STATUS###' );
        switch ($this->conf ['monitor']) {
          ...
          case 'start': /* Subscribe a user */
              ...
              $status = $this->templateService->getSubpart( $page, '###STATUS_START###' );
      ...
      if ($this->conf ['subscribeFeUser'] && $this->rightsObj->isLoggedIn()) {
        $subscriptionContainer = $this->templateService->getSubpart( $page, '###SUBSCRIPTION_CONTAINER###' );
        $return = '';
  ...
  function unsubscribeByTable($table, $email, $eventUID, $subscriptionHash) {
    ...
    $sqlWhere = 'tx_cal_event.uid = ' . $eventUID . ' AND tx_cal_event.pid IN (' . $this->conf ['pidList'] . ')' . $this->pageRepository->enableFields( 'tx_cal_event' );
