...
class DeleteEventView extends \TYPO3\CMS\Cal\View\FeEditingBaseView {
  ...
  function getExceptionMarker(& $template, & $sims, & $rems) {
    ...
    $exception = Array ();
    $result = $GLOBALS ['TYPO3_DB']->exec_SELECTquery( 'tx_cal_exception_event.title', 'tx_cal_exception_event,tx_cal_exception_event_mm', 'pid IN (' . $this->conf ['pidList'] . ') AND tx_cal_exception_event_mm.tablenames = "tx_cal_exception_event" AND tx_cal_exception_event_mm.uid_local =' . $this->object->getUid() . $this->pageRepository->enableFields( 'tx_cal_exception_event' ) );
    while ( $row = $GLOBALS ['TYPO3_DB']->sql_fetch_assoc( $result ) ) {
      $exception [] = $row ['title'];
    }
    $GLOBALS ['TYPO3_DB']->sql_free_result( $result );
    
    $result = $GLOBALS ['TYPO3_DB']->exec_SELECTquery( 'tx_cal_exception_event_group.title', 'tx_cal_exception_event_group,tx_cal_exception_event_mm', 'pid in (' . $this->conf ['pidList'] . ') AND tx_cal_exception_event_mm.tablenames = "tx_cal_exception_event_group" AND tx_cal_exception_event_mm.uid_local =' . $this->object->getUid() . $this->pageRepository->enableFields( 'tx_cal_exception_event_group' ) );
    while ( $row = $GLOBALS ['TYPO3_DB']->sql_fetch_assoc( $result ) ) {
      $exception [] = $row ['title'];
    }
    $GLOBALS ['TYPO3_DB']->sql_free_result( $result );
  ...
  function getCreateExceptionMarker(& $template, & $sims, & $rems) {
    ...
    if ($this->object->isClone() && $this->rightsObj->isAllowedToCreateExceptionEvent()) {
      ...
      $rems ['###CREATE_EXCEPTION###'] = $this->templateService->getSubpart( $template, '###CREATE_EXCEPTION###' );
