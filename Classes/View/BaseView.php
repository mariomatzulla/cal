...
class BaseView extends \TYPO3\CMS\Cal\Service\BaseService {
  ...
  public function __construct() {
    ...
    $this->templateService = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance( \TYPO3\CMS\Core\Service\MarkerBasedTemplateService::class );
  }
  ...
  function getTomorrowsEventsMarker(&$page, &$sims, &$rems, &$wrapped) {
    ...
    if ($this->conf ['view.'] ['other.'] ['showTomorrowEvents'] == 1) {
      $rems ['###TOMORROWS_EVENTS###'] = $this->tomorrows_events( $this->templateService->getSubpart( $page, '###TOMORROWS_EVENTS###' ) );
    }
  }
  ...
  function getTodoMarker(&$page, &$sims, &$rems, &$wrapped) {
    ...
    if ($this->rightsObj->isViewEnabled( 'todo' ) && $confArr ['todoSubtype'] == 'todo') {
      ...
      $todoTemplate = $this->templateService->getSubpart( $page, '###TODO###' );
      $rems ['###TODO###'] = \TYPO3\CMS\Cal\Utility\Functions::substituteMarkerArrayNotCached( $todoTemplate, array (), array (
  ...
  function getUserLoginMarker(&$page, &$sims, &$rems, &$wrapped) {
    ...
    if ($this->conf ['view.'] ['other.'] ['showLogin'] == 1) {
      ...
      $rems ['###USER_LOGIN###'] = \TYPO3\CMS\Cal\Utility\Functions::substituteMarkerArrayNotCached( $this->templateService->getSubpart( $page, '###USER_LOGIN###' ), $local_sims, $local_rems, array () );
    }
  }
  ...
  function getSearchMarker(&$page, &$sims, &$rems, &$wrapped) {
    ...
    if ($this->conf ['view.'] ['other.'] ['showSearch'] == 1) {
      ...
      $rems ['###SEARCH###'] = \TYPO3\CMS\Cal\Utility\Functions::substituteMarkerArrayNotCached( $this->templateService->getSubpart( $page, '###SEARCH###' ), $local_sims, array (), array () );
    }
  }
  ...
  function getJumpsMarker(&$page, &$sims, &$rems, &$wrapped) {
    ...
    if ($this->conf ['view.'] ['other.'] ['showJumps'] == 1) {
      ...
      $rems ['###JUMPS###'] = \TYPO3\CMS\Cal\Utility\Functions::substituteMarkerArrayNotCached( $this->templateService->getSubpart( $page, '###JUMPS###' ), $temp_sims, array (), array () );
    }
  }
  ...
  function getCalendarSelectorMarker(&$page, &$sims, &$rems, &$wrapped) {
    ...
    if ($this->conf ['view.'] ['other.'] ['showCalendarSelection']) {
      ...
      $change_calendar_action_url = $this->controller->pi_linkTP_keepPIvars_url( array (
      ...
      $rems ['###CALENDAR_SELECTOR###'] = \TYPO3\CMS\Cal\Utility\Functions::substituteMarkerArrayNotCached( $this->templateService->getSubpart( $page, '###CALENDAR_SELECTOR###' ), $temp_sims, array (), array () );
    }
  }
  ...
  function getListMarker(&$page, &$sims, &$rems, &$wrapped) {
    ...
    $tx_cal_listview = GeneralUtility::makeInstanceService( 'cal_view', 'list', [ 
  ...
  function getRelatedListMarker(&$page, &$sims, &$rems, &$wrapped) {
    ...
    $listSubpart = $this->templateService->getSubpart( $page, '###RELATED_LIST###' );
  ...
  function getCategoryurlMarker(&$page, &$sims, &$rems, $view) {
    ...
    $sims ['###CATEGORYURL###'] = htmlspecialchars( $this->controller->pi_linkTP_keepPIvars_url( array (
  ...
  function replaceViewMarker($page) {
    ...
    // next day
    $nextdaylinktext = $this->templateService->getSubpart( $page, '###NEXT_DAYLINKTEXT###' );
    ...
    // prev day
    $prevdaylinktext = $this->templateService->getSubpart( $page, '###PREV_DAYLINKTEXT###' );
    ...
    // next week
    $nextweeklinktext = $this->templateService->getSubpart( $page, '###NEXT_WEEKLINKTEXT###' );
    ...
    // prev week
    $prevweeklinktext = $this->templateService->getSubpart( $page, '###PREV_WEEKLINKTEXT###' );
    ...
    // next month
    $nextmonthlinktext = $this->templateService->getSubpart( $page, '###NEXT_MONTHLINKTEXT###' );
    ...
    // prev month
    $prevmonthlinktext = $this->templateService->getSubpart( $page, '###PREV_MONTHLINKTEXT###' );
    ...
    // next year
    $nextyearlinktext = $this->templateService->getSubpart( $page, '###NEXT_YEARLINKTEXT###' );
    ...
    // prev year
    $prevyearlinktext = $this->templateService->getSubpart( $page, '###PREV_YEARLINKTEXT###' );
  ...
  function replace_files($page, $tags = array ()) {
    ...
    if (sizeof( $tags ) > 0)
      foreach ( $tags as $tag => $data ) {
        ...
        $data = $filePathSanitizer->sanitize( $data );
  ...
  function getViewLinkMarker($view, &$template, &$sims, &$rems, &$wrapped) {
    ...
    if ($this->rightsObj->isViewEnabled( $viewTarget ) || $this->conf ['view.'] [$viewTarget . '.'] [$viewTarget . 'ViewPid']) {
      ...
      $this->local_cObj->setCurrentVal( $this->templateService->getSubpart( $template, '###' . strtoupper( $view ) . 'VIEWLINKTEXT###' ) );
  ...
  function tomorrows_events($template) {
    ...
    $match1 = $this->templateService->getSubpart( $template, '###T_ALLDAY_SWITCH###' );
    $match2 = $this->templateService->getSubpart( $template, '###T_EVENT_SWITCH###' );
  ...
  function _draw_month($page, $offset = '+0', $type) {
    ...
    if ($monthTemplate != '') {
      $loop_wd = $this->templateService->getSubpart( $monthTemplate, '###LOOPWEEKDAY###' );
      $t_month = $this->templateService->getSubpart( $monthTemplate, '###SWITCHMONTHDAY###' );
      $startweek = $this->templateService->getSubpart( $monthTemplate, '###LOOPMONTHWEEKS_DAYS###' );
      $endweek = $this->templateService->getSubpart( $monthTemplate, '###LOOPMONTHDAYS_WEEKS###' );
      $weeknum = $this->templateService->getSubpart( $monthTemplate, '###LOOPWEEK_NUMS###' );
      $corner = $this->templateService->getSubpart( $monthTemplate, '###CORNER###' );
    ...
    $listTemplate = $this->templateService->getSubpart( $page, '###LIST###' );
    if ($listTemplate != '') {
      $tx_cal_listview = &GeneralUtility::makeInstanceService( 'cal_view', 'list', [ 
   ...
   *          Array with key => value pairs that should be used as fake db-values for TS rendering instead of the values of the current object
   */
  ...
  function renderWithFluid($object = null) {
    ...
    $view->setPartialRootPaths( [ 
        ...
        GeneralUtility::getFileAbsFileName( $this->conf ['fluidPartialsPath'] )
    ] );
