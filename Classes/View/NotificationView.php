...
class NotificationView extends \TYPO3\CMS\Cal\Service\BaseService {
  ...
  function sendNotificationOfChanges(&$event_old, &$event_new, $email, $templatePath, $titleText, $unsubscribeLink, $acceptLink = '', $declineLink = '') {
    ...
    $htmlTemplate = $this->templateService->getSubpart( $template, '###HTML###' );
    $oldEventHTMLSubpart = $this->templateService->getSubpart( $htmlTemplate, '###OLD_EVENT###' );
    $newEventHTMLSubpart = $this->templateService->getSubpart( $htmlTemplate, '###NEW_EVENT###' );
    
    $plainTemplate = $this->templateService->getSubpart( $template, '###PLAIN###' );
    $oldEventPlainSubpart = $this->templateService->getSubpart( $plainTemplate, '###OLD_EVENT###' );
    $newEventPlainSubpart = $this->templateService->getSubpart( $plainTemplate, '###NEW_EVENT###' );
  ...
  function getModifyingUser($template) {
    ...
    $currentUserSubpart = $this->templateService->getSubpart( $template, '###CURRENT_USER_SUBPART###' );
