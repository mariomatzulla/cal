...
abstract class NewTimeView {
  ...
  public function render(&$template) {
    ...
    $subpart = $this->templateService->getSubpart( $template, $this->getMySubpart() );
  ...
  public function getWeekdayString($timestamp) {
    ...
    $conf = &\TYPO3\CMS\Cal\Utility\Registry::Registry( 'basic', 'conf' );
    return mb_substr( strftime( $this->getWeekDayFormat(), $timestamp ), 0, $this->getWeekDayLength(), \TYPO3\CMS\Cal\Utility\Functions::getCharset() );
  }
  ...
  public function getTimeCellsMarker(& $template, & $sims, & $rems, & $wrapped, $view) {
    ...
    $rightsObj = &\TYPO3\CMS\Cal\Utility\Registry::Registry( 'basic', 'rightscontroller' );
    $timesSubpart = $this->templateService->getSubpart( $template, 'TIME_CELLS' );
  ...
  public function getHourCellsMarker(& $template, & $sims, & $rems, & $wrapped, $view) {
    ...
    $hoursSubpart = $this->templateService->getSubpart( $template, 'HOUR_CELLS' );
