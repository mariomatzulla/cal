...
class WeekView extends \TYPO3\CMS\Cal\View\BaseView {
  ...
  public function drawWeek(&$master_array, $getdate) {
    ...
    $weekTemplate = $this->templateService->getSubpart( $page, '###WEEK_TEMPLATE###' );
    if ($weekTemplate == '') {
      $rems = Array ();
      return $this->finish( $page, $rems );
    }
    ...
    $alldays = $this->templateService->getSubpart( $weekTemplate, '###ALLDAYSOFWEEK##' );
    ...
    $loop_dof = $this->templateService->getSubpart( $weekTemplate, '###DAYSOFWEEK###' );
