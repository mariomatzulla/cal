...
class CreateEventView extends \TYPO3\CMS\Cal\View\FeEditingBaseView {
  ...
  function prepareUserArray() {
    ...
    if ($this->isEditMode) {
      ...
      $where = ' AND tx_cal_event.uid=' . $this->object->getUid() . ' AND fe_users.deleted = 0 AND fe_users.disable = 0' . $this->pageRepository->enableFields( 'tx_cal_event' );
      ...
      $where = ' AND tx_cal_event.uid=' . $this->object->getUid() . ' AND tx_cal_fe_user_event_monitor_mm.tablenames="fe_groups" ' . $this->pageRepository->enableFields( 'tx_cal_event' );
  ...
  function getNotifyMarker(& $template, & $sims, & $rems) {
    ...
    if ($this->isAllowed( 'notify' )) {
      ...
      $result = $GLOBALS ['TYPO3_DB']->exec_SELECTquery( '*', 'fe_users', $pidWhere . $allowedUsersWhere . $this->pageRepository->enableFields( 'fe_users' ) );
      while ( $row = $GLOBALS ['TYPO3_DB']->sql_fetch_assoc( $result ) ) {
      ...
      $result = $GLOBALS ['TYPO3_DB']->exec_SELECTquery( '*', 'fe_groups', $pidWhere . $allowedGroupsWhere . $this->pageRepository->enableFields( 'fe_groups' ) );
      while ( $row = $GLOBALS ['TYPO3_DB']->sql_fetch_assoc( $result ) ) {
  ...
  function getExceptionMarker(& $template, & $sims, & $rems) {
    ...
    if ($this->rightsObj->isAllowedToCreateEventException() || $this->rightsObj->isAllowedToEditEventException()) {
      ...
      $exception = '';
      $result = $GLOBALS ['TYPO3_DB']->exec_SELECTquery( '*', 'tx_cal_exception_event', 'pid in (' . $this->conf ['pidList'] . ')' . $this->pageRepository->enableFields( 'tx_cal_exception_event' ) );
      while ( $row = $GLOBALS ['TYPO3_DB']->sql_fetch_assoc( $result ) ) {
      ...
      $result = $GLOBALS ['TYPO3_DB']->exec_SELECTquery( '*', 'tx_cal_exception_event_group', 'pid in (' . $this->conf ['pidList'] . ')' . $this->pageRepository->enableFields( 'tx_cal_exception_event_group' ) );
      while ( $row = $GLOBALS ['TYPO3_DB']->sql_fetch_assoc( $result ) ) {
  ...
  function getFormStartMarker(& $template, & $sims, & $rems, & $wrapped) {
    ...
    $temp = $this->templateService->getSubpart( $template, '###FORM_START###' );
    $temp_sims = Array ();
  ...
  function getAttendeeMarker(& $template, & $sims, & $rems) {
    ...
    if ($this->isAllowed( 'attendee' )) {
      ...
      $result = $GLOBALS ['TYPO3_DB']->exec_SELECTquery( '*', 'fe_users', 'pid in (' . $this->conf ['pidList'] . ')' . $this->pageRepository->enableFields( 'fe_users' ) );
      while ( $row = $GLOBALS ['TYPO3_DB']->sql_fetch_assoc( $result ) ) {
        ...
        if ($this->conf ['view.'] [$this->conf ['view'] . '.'] ['freeAndBusyViewPid'] && $row ['tx_cal_calendar'] && $this->rightsObj->isLoggedIn()) {
          ...
          $result1 = $GLOBALS ['TYPO3_DB']->exec_SELECTquery( '*', 'tx_cal_calendar', 'pid in (' . $this->conf ['pidList'] . ') AND uid =' . $row ['tx_cal_calendar'] . $this->pageRepository->enableFields( 'tx_cal_calendar' ) );
          while ( $row1 = $GLOBALS ['TYPO3_DB']->sql_fetch_assoc( $result1 ) ) {
