...
class UriHandler {
  ...
  // This is a prefix that will be added to all links in the mail. Example: 'http://www.mydomain.com/jump?userid=###FIELD_uid###&url='. if used, anything after url= is urlencoded.
    ...
    if (is_array( $this->theParts ['html'] ['media'] )) {
      foreach ( $this->theParts ['html'] ['media'] as $key => $val ) {
        if ($val ['use_jumpurl'] && $this->jumperURL_prefix) {
          $subst = $this->jumperURL_prefix . Functions::rawUrlEncodeFP( $val ['absRef'] );
        } else {
    ...
    foreach ( $this->theParts ['html'] ['hrefs'] as $key => $val ) {
      ...
      if ($this->jumperURL_prefix && $val ['tag'] != 'form') {
        if ($this->jumperURL_useId) {
          ...
          $substVal = $this->jumperURL_prefix . Functions::rawUrlEncodeFP( $val ['absRef'] );
        }
      } else {
        $substVal = $val ['absRef'];
      }
  ...
  public function getMimeType($url) {
    ...
    $mimeType = '';
    $requestFactory = static::makeInstance( \TYPO3\CMS\Core\Http\RequestFactory::class );
    ...
    if ($includeHeader) {
      ...
      $content = $method . ' ' . ($parsedURL ['path'] ?? '/') . (! empty( $parsedURL ['query'] ) ? '?' . $parsedURL ['query'] : '') . ' HTTP/1.0' . CRLF . 'Host: ' . $parsedURL ['host'] . CRLF . 'Connection: close' . CRLF;
      if (is_array( $requestHeaders )) {
