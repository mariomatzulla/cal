...
class Controller extends \TYPO3\CMS\Frontend\Plugin\AbstractPlugin {
  ...
  // Same as class name
  var $scriptRelPath = 'Classes/Controller/Controller.php';
  ...
  /**
   * Validates that various date piVars are within valid ranges.
   * Any dates outside a valid range have
   * their piVars unset.
   */
   ...
   *
   * @param
   *          array The array of hook objects.
   * @param
  ...
  /**
   * Clears $this-conf vars related to view and lastview.
   * Useful when calling save and remove functions.
   */
  ...
  public function translateLanguageMarker(&$content) {
    ...
    if (count( $allLanguageMarkers )) {
      ...
      if (count( $sims )) {
        ...
        $content = $templateService->substituteMarkerArray( $content, $sims );
      }
    }
    return $content;
  }
