...
class CalendarService extends \TYPO3\CMS\Cal\Service\BaseService {
  ...
  function getCalendarSearchString($pidList, $includePublic, $linkIds) {
    ...
    $calendarSearchString .= $this->pageRepository->enableFields( 'tx_cal_calendar' ) . ' AND tx_cal_calendar.pid IN (' . $pidList . ') ';
  ...
  function getIdsFromTable($list, $pidList, $includePublic, $includeData = false, $onlyPublic = false) {
    ...
    $orderBy = \TYPO3\CMS\Cal\Utility\Functions::getOrderBy( 'tx_cal_calendar' );
    $result = $GLOBALS ['TYPO3_DB']->exec_SELECTquery( 'tx_cal_calendar_user_group_mm.uid_local', 'tx_cal_calendar_user_group_mm LEFT JOIN tx_cal_calendar ON tx_cal_calendar.uid=tx_cal_calendar_user_group_mm.uid_local', '1=1 ' . $this->pageRepository->enableFields( 'tx_cal_calendar' ), '', $orderBy );
    if ($result) {
      while ( $row = $GLOBALS ['TYPO3_DB']->sql_fetch_assoc( $result ) ) {
        $ids [] = $row ['uid_local'];
      }
    ...
    if ($includePublic) {
      ...
      if (! empty( $ids )) {
        $where = 'uid NOT IN (' . implode( ',', $ids ) . ') ' . $this->pageRepository->enableFields( 'tx_cal_calendar' );
      } else {
        $where = '0=0 ' . $this->pageRepository->enableFields( 'tx_cal_calendar' );
      }
      if ($pidList != '') {
    ...
    if (! $onlyPublic) {
      ...
      if ($where == '') {
        ...
        $where .= $this->pageRepository->enableFields( 'tx_cal_calendar' );
      }
      if ($limitationList != '') {
