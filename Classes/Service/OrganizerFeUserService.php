...
class OrganizerFeUserService extends \TYPO3\CMS\Cal\Service\BaseService {
  ...
  public function find($uid, $pidList) {
    ...
    if (! $this->isAllowedService())
      return;
    if ($pidList == '') {
      $result = $GLOBALS ['TYPO3_DB']->exec_SELECTquery( '*', 'fe_users', 'uid=' . $uid . ' ' . $this->pageRepository->enableFields( 'fe_users' ) );
    } else {
      $result = $GLOBALS ['TYPO3_DB']->exec_SELECTquery( '*', 'fe_users', ' pid IN (' . $pidList . ') AND uid=' . $uid . ' ' . $this->pageRepository->enableFields( 'fe_users' ) );
    }
    if ($result) {
  ...
  public function findAll($pidList) {
    ...
    if ($pidList == '') {
      $result = $GLOBALS ['TYPO3_DB']->exec_SELECTquery( '*', 'fe_users', ' 1 = 1 ' . $this->pageRepository->enableFields( 'fe_users' ), '', $orderBy );
    } else {
      $result = $GLOBALS ['TYPO3_DB']->exec_SELECTquery( '*', 'fe_users', ' pid IN (' . $pidList . ') ' . $this->pageRepository->enableFields( 'fe_users' ), '', $orderBy );
    }
    if ($result) {
  ...
  private function getOrganizerFromTable($pidList = '', $additionalWhere = '') {
    ...
    $select = $this->tableId . '.*';
    $table = $this->tableId;
    $where = '1=1 ' . $additionalWhere . $this->pageRepository->enableFields( $this->tableId );
    $groupBy = '';
    $orderBy = \TYPO3\CMS\Cal\Utility\Functions::getOrderBy( $this->tableId );
    $limit = '';
  ...
  private function retrievePostData(&$insertFields) {
    ...
    if ($this->rightsObj->isAllowedToEditOrganizerDescription() || $this->rightsObj->isAllowedToCreateOrganizerDescription()) {
      $insertFields ['title'] = Functions::removeBadHTML( $this->controller->piVars ['title'], $this->conf );
    }
  ...
  public function saveOrganizer($pid) {
    ...
    if ($this->controller->piVars ['description'] != '') {
      $insertFields ['title'] = Functions::removeBadHTML( $this->controller->piVars ['title'] );
    }
