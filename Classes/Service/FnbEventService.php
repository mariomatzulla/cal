...
class FnbEventService extends \TYPO3\CMS\Cal\Service\EventService {
  ...
  function getIdsFromTable($list, $pidList, $includePublic, $includeData = false, $onlyPublic = false) {
    ...
    $result = $GLOBALS ['TYPO3_DB']->exec_SELECTquery( 'tx_cal_calendar_fnb_user_group_mm.uid_local', 'tx_cal_calendar_fnb_user_group_mm LEFT JOIN tx_cal_calendar ON tx_cal_calendar.uid=tx_cal_calendar_fnb_user_group_mm.uid_local', '1=1 ' . $this->pageRepository->enableFields( 'tx_cal_calendar' ), '', $orderBy );
    if ($result) {
      while ( $row = $GLOBALS ['TYPO3_DB']->sql_fetch_assoc( $result ) ) {
        $ids [] = $row ['uid_local'];
      }
    ...
    if ($includePublic) {
      ...
      $where .= 'tx_cal_calendar.activate_fnb = 1 ' . $this->pageRepository->enableFields( 'tx_cal_calendar' );
      if ($pidList != '') {
