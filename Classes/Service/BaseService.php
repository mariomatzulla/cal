...
abstract class BaseService extends \TYPO3\CMS\Core\Service\AbstractService {
  ...
  public function __construct() {
    ...
    $this->extConf = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance( \TYPO3\CMS\Core\Configuration\ExtensionConfiguration::class )->get( 'cal' );
    if (! isset( $this->extConf ['categoryService'] )) {
      $this->extConf ['categoryService'] = 'tx_cal_category';
    }
    $this->pageRepository = new \TYPO3\CMS\Core\Domain\Repository\PageRepository();
  }
  ...
  protected function checkOnTempFile($type, &$insertFields, $objectType, $uid) {
    ...
    $result = $GLOBALS ['TYPO3_DB']->exec_UPDATEquery( $objectType, 'uid = ' . $uid, Array (
