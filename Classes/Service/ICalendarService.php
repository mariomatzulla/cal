...
class ICalendarService extends \TYPO3\CMS\Cal\Service\BaseService {
  ...
  public function find($uid, $pidList = '') {
    ...
    if (TYPO3_MODE == 'BE') {
      ...
      $enableFields = $this->pageRepository->enableFields( 'tx_cal_calendar' );
    }
    if ($pidList == '') {
  ...
  public function scheduleUpdates($refreshInterval, $uid) {
    ...
    if (\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::isLoaded( 'scheduler' )) {
      ...
      if ($recurring) {
        $calendarRow = BackendUtility::getRecord( 'tx_cal_calendar', $uid );
  ...
  public function createSchedulerTask(&$scheduler, $offset, $calendarUid) {
    ...
    $taskGroup = [ ];
    $result = $GLOBALS ['TYPO3_DB']->exec_SELECTquery( '*', 'tx_scheduler_task_group', 'groupName="cal"' );
    if ($result) {
      $taskGroup = $GLOBALS ['TYPO3_DB']->sql_fetch_assoc( $result );
      $GLOBALS ['TYPO3_DB']->sql_free_result( $result );
    }
    if ($taskGroup ['uid']) {
  ...
  public function deleteSchedulerTask($calendarUid) {
    ...
    if (\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::isLoaded( 'scheduler' )) {
      $calendarRow = [ ];
      $result = $GLOBALS ['TYPO3_DB']->exec_SELECTquery( '*', 'tx_cal_calendar', 'uid=' . $calendarUid );
      if ($result) {
        $calendarRow = $GLOBALS ['TYPO3_DB']->sql_fetch_assoc( $result );
        $GLOBALS ['TYPO3_DB']->sql_free_result( $result );
      }
      $taskId = $calendarRow ['schedulerId'];
      if ($taskId > 0) {
  ...
  private function getDateValue($component, $attribute) {
    ...
    if ($component->getAttribute( $attribute )) {
      ...
      if ($timezone) {
        $dateTime->setTZ( $timezone );
        if ($GLOBALS ['TYPO3_CONF_VARS'] ['SYS'] ['phpTimeZone']) {
          $dateTime->convertTZbyID( $GLOBALS ['TYPO3_CONF_VARS'] ['SYS'] ['phpTimeZone'] );
        } else {
          $dateTime->convertTZbyID( 'UTC' );
        }
      }
      return $dateTime;
    }
  ...
  private function setRecurrenceId($component, &$insertFields) {

    $uid = $component->getAttribute( 'UID' );
    $result = $GLOBALS ['TYPO3_DB']->exec_SELECTquery( '*', 'tx_cal_event', 'icsUid="' . $uid . '"' );
    if ($result) {
      $row = $GLOBALS ['TYPO3_DB']->sql_fetch_assoc( $result );
      $eventUid = $row ['uid'];
      $GLOBALS ['TYPO3_DB']->sql_free_result( $result );
    ...
    $indexEntry = [ ];
    $result = $GLOBALS ['TYPO3_DB']->exec_SELECTquery( '*', 'tx_cal_index', 'event_uid="' . $eventUid . '" AND start_datetime="' . $recurrenceIdStart->format( '%Y%m%d%H%M%S' ) . '"' );
    if ($result) {
      $indexEntry = $GLOBALS ['TYPO3_DB']->sql_fetch_assoc( $result );
      $GLOBALS ['TYPO3_DB']->sql_free_result( $result );
    }
    
    if ($indexEntry) {
      $table = 'tx_cal_event_deviation';
      unset( $insertFields ['ext_url'] );
      unset( $insertFields ['attendee'] );
      $insertFields ['imagecaption'] = '';
      $insertFields ['imagealttext'] = '';
      $insertFields ['imagetitletext'] = '';
      $insertFields ['attachmentcaption'] = '';
  ...
  private function generateIndexEntries($eventUid, $pageIdForPlugin) {
  ...
  public function clearAllImagesAndAttachments($uid) {
    ...
    $fileIndexRepository = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance( 'TYPO3\\CMS\\Core\\Resource\\Index\\FileIndexRepository', $eventDispatcher );
  ...
  public function insertCalEventsIntoDB($iCalendarComponentArray = array(), $calId, $pid = '', $cruserId = '', $isTemp = 1, $deleteNotUsedCategories = true) {
    ...
    foreach ( $iCalendarComponentArray as $component ) {
      ...
      $insertFields ['l18n_diffsource'] = '';
      $insertFields ['ext_url'] = '';
      $insertFields ['image'] = '';
      $insertFields ['attachment'] = '';
      $insertFields ['attendee'] = '';
      $insertFields ['pid'] = $pid;
      if ($component->getType() == 'vEvent' || $component->getType() == 'vTodo') {
        ...
        $result = $GLOBALS ['TYPO3_DB']->exec_SELECTquery( '*', 'tx_cal_event', 'icsUid="' . $insertFields ['icsUid'] . '"' );
        if ($result) {
    ...
    foreach ($insertedOrUpdatedEventUids as $eventUid){
      $this->sendReminders( $eventUid, $pid );
    }
   ...
   */
  public static function insertRuleValues($rule, &$insertFields) {
  ...
  private function createExceptionRule($pid, $cruserId, $eventUid, $exceptionRuleDescription) {
    ...
    $event = BackendUtility::getRecord( 'tx_cal_event', 'uid=' . $eventUid );
