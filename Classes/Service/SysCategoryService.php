...
class SysCategoryService extends \TYPO3\CMS\Cal\Service\BaseService {
  ...
  public function getCategoryArray($pidList, &$categoryArrayToBeFilled, $showPublicCategories = true) {
    ...
    if (isset($this->conf ['view.'] ['allowedCategories']) && ! empty( $this->categoryArrayCached [md5( $this->conf ['view.'] ['categoryMode'] . $this->conf ['view.'] ['allowedCategories'] )] )) {
    ...
    $where .= $this->pageRepository->enableFields( 'tx_cal_calendar' ) . ' AND tx_cal_calendar.pid IN (' . $pidList . ') ' . $this->pageRepository->enableFields( 'sys_category' );
    ...
    if ($result) {
      while ( $row = $GLOBALS ['TYPO3_DB']->sql_fetch_assoc( $result ) ) {
        if ($languageAspect->getContentId()) {
          $row = $GLOBALS ['TSFE']->sys_page->getRecordOverlay( 'sys_category', $row, $languageAspect->getContentId(), $languageAspect->getLegacyOverlayType(), '' );
        }
        if (! $row ['uid']) {
          continue;
        }
    ...
    if (! empty( $calendarsWithoutCategory )) {
      ...
      $where = 'tx_cal_calendar.uid IN (' . implode( ',', $calendarsWithoutCategory ) . ')' . $calendarSearchString . $this->pageRepository->enableFields( 'tx_cal_calendar' );
    ...
    $where = 'sys_category.calendar_id = 0' . $this->pageRepository->enableFields( 'sys_category' ) . $additionalWhere;
    $where .= $this->getAdditionalWhereForLocalizationAndVersioning( 'sys_category' );
    ...
    if ($result) {
      while ( $row = $GLOBALS ['TYPO3_DB']->sql_fetch_assoc( $result ) ) {
        ...
        /**
         * FIXME versioningPreview not public available anymore
         if ($GLOBALS ['TSFE']->sys_page->versioningPreview == TRUE) {
          // get workspaces Overlay
          $GLOBALS ['TSFE']->sys_page->versionOL( 'sys_category', $row );
        }*/
    ...
    if ($this->conf ['view.'] ['freeAndBusy.'] ['enable']) {
      ...
      $where .= $this->pageRepository->enableFields( 'tx_cal_calendar' ) . $this->pageRepository->enableFields( 'sys_category' ) . $this->pageRepository->enableFields( 'tx_cal_event' );
      $where .= $this->getAdditionalWhereForLocalizationAndVersioning( 'sys_category' );
    ...
    if ($categoryStringByUid && isset($this->conf ['view.'] ['categoryMode'])) {
  ...
  private function getAllCategoryIdsByParentId() {
    ...
    if ($this->allCateogryIdsByParentId == null) {
      ...
      $table = 'sys_category';
      $where = '1=1' . $this->pageRepository->enableFields( 'sys_category' );
      $groupby = '';
  ...
  public function getCategoriesForSharedUser() {
    ...
    $select = '*';
    $table = 'tx_cal_event LEFT JOIN tx_cal_event_shared_user_mm ON tx_cal_event.uid = tx_cal_event_shared_user_mm.uid_local ' . 'LEFT JOIN tx_cal_calendar ON tx_cal_event.calendar_id = tx_cal_calendar.uid ' . 'LEFT JOIN sys_category ON tx_cal_calendar.uid = sys_category.calendar_id';
    $where = 'sys_category.shared_user_allowed = 1' . ' AND tx_cal_event_shared_user_mm.uid_foreign = ' . $this->rightsObj->getUserId() . $this->pageRepository->enableFields( 'tx_cal_calendar' ) . $this->pageRepository->enableFields( 'sys_category' ) . $this->pageRepository->enableFields( 'tx_cal_event' );
