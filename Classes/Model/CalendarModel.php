...
class CalendarModel extends \TYPO3\CMS\Cal\Model\BaseModel {
  ...
  private function init(&$row) {
    ...
    $result = $GLOBALS ['TYPO3_DB']->exec_SELECTquery( 'tx_cal_calendar_fnb_user_group_mm.*', 'tx_cal_calendar_fnb_user_group_mm, fe_users, fe_groups', 
        // join on fe_users
        '((uid_foreign = fe_users.uid AND tablenames="fe_users" ' . $pageRepository->enableFields( 'fe_users' ) . ') OR ' . 
        // join on fe_groups
        '(uid_foreign = fe_groups.uid AND tablenames="fe_groups" ' . $pageRepository->enableFields( 'fe_groups' ) . ')) AND ' . 
        // general conditions
  ...
  public function getEditLink(& $template, & $sims, & $rems, $view) {
    ...
    if ($this->isUserAllowedToEdit()) {
      ...
      $this->controller->getParametersForTyposcriptLink( $this->local_cObj->data, array (
          ...
          'type' => $this->getType(),
          'uid' => $this->getUid()
      ), $this->conf ['cache'], $this->conf ['clear_anyway'] ?? false, $this->conf ['view.'] ['calendar.'] ['editCalendarViewPid'] );
    ...
    if ($this->isUserAllowedToDelete()) {
      ...
      $this->controller->getParametersForTyposcriptLink( $this->local_cObj->data, array (
          ...
          'type' => $this->getType(),
          'uid' => $this->getUid()
      ), $this->conf ['cache'], $this->conf ['clear_anyway'] ?? false, $this->conf ['view.'] ['calendar.'] ['deleteCalendarViewPid'] );
