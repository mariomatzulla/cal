...
class EventModel extends \TYPO3\CMS\Cal\Model\Model {
    ...
    foreach ( $piVars as $key => $value ) {
      switch ($key) {
        ...
        case 'title' :
          ...
          break;
        case 'description' :
          $this->setDescription( Functions::removeBadHTML( $piVars ['description'] ) );
          unset( $piVars ['description'] );
          unset( $piVars ['_TRANSFORM_description'] );
          break;
        case 'teaser' :
          $this->setTeaser( Functions::removeBadHTML( $piVars ['teaser'] ) );
          unset( $piVars ['teaser'] );
          break;
        case 'image' :
  ...
  function fillTemplate($subpartMarker) {
    ...
    $page = $templateService->getSubpart( $page, $subpartMarker );
    if (! $page) {
  ...
  function getSubscriptionMarker(& $template, & $sims, & $rems, &$wrapped, $view) {
    ...
    if (($this->conf ['allowSubscribe'] == 1 || ($this->conf ['subscribeFeUser'] == 1 && $rightsObj->isLoggedIn())) && $uid) {
          ...
          case 'start' :
            {
              if ($user_uid > 0) {
                ...
                if (($captchaStr && $this->controller->piVars ['captcha'] === $captchaStr) || ($this->conf ['subscribeWithCaptcha'] == 0)) {
                  ...
                  $templateService = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance( \TYPO3\CMS\Core\Service\MarkerBasedTemplateService::class );
                  $htmlTemplate = $templateService->getSubpart( $local_template, '###HTML###' );
                  $plainTemplate = $templateService->getSubpart( $local_template, '###PLAIN###' );
      ...
      if ($rightsObj->isLoggedIn()) {
        ...
        if ($this->conf ['subscribeWithCaptcha'] == 1 && \TYPO3\CMS\Core\Utility\ExtensionManagementUtility::isLoaded( 'captcha' )) {
          $sims_temp ['CAPTCHA_SRC'] = '<img src="' . \TYPO3\CMS\Core\Utility\PathUtility::stripPathSitePrefix(\TYPO3\CMS\Core\Utility\ExtensionManagementUtility::extPath('captcha')) . 'captcha/captcha.php' . '" alt="" />';
        ...
        $templateService = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance( \TYPO3\CMS\Core\Service\MarkerBasedTemplateService::class );
        $notLoggedinNoMonitoring = $templateService->getSubpart( $template, '###NOTLOGGEDIN_NOMONITORING###' );
        $parameter = array (
        ...
        $notLoggedinMonitoring = $templateService->getSubpart( $template, '###NOTLOGGEDIN_MONITORING###' );
        ...
        foreach ( $globalAttendeeArray as $serviceKey => $attendeeArray ) {
          foreach ( $attendeeArray as $attendee ) {
            if ($attendee->getFeUserId()) {
              $pageRepository = new \TYPO3\CMS\Core\Domain\Repository\PageRepository();
              $result = $GLOBALS ['TYPO3_DB']->exec_SELECTquery( '*', 'fe_users', 'pid in (' . $this->conf ['pidList'] . ')' . $pageRepository->enableFields( 'fe_users' ) . ' AND uid =' . $attendee->getFeUserId() );
              if ($result) {
                while ( $row = $GLOBALS ['TYPO3_DB']->sql_fetch_assoc( $result ) ) {
  ...
  function getGuidMarker(& $template, & $sims, & $rems, & $wrapped, $view) {
    ...
    if ($this->row ['icsUid'] != '') {
      ...
      $eventArray = array (
          ...
          'calendar_id' => $this->getCalendarUid(),
          'uid' => $this->getUid()
      );
  ...
  function getRRule(&$event) {
    ...
    if (in_array( $event->getFreq(), $allowedValues )) {
      ...
      if (is_object( $until ) && $until->format( '%Y%m%d' ) > 19700101) {
        ...
        $offset = \TYPO3\CMS\Cal\Utility\Functions::strtotimeOffset( $eventEnd->getTime() );
        $eventEnd->subtractSeconds( $offset );
        $rruleConfiguration ['UNTIL'] = 'UNTIL=' . $until->format( '%Y%m%d' ) . 'T000000Z';
        $eventEnd->addSeconds( $offset );
      }
